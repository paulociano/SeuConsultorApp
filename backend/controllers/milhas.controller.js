...
const getCarteiras = async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM milhas_carteiras WHERE user_id = $1 ORDER BY tipo, nome',
      [req.usuario.id]
    );
...
const createCarteira = async (req, res) => {
  ...
  const sql =
    'INSERT INTO milhas_carteiras (user_id, nome, saldo, cpm_medio, data_expiracao, tipo) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *';
  try {
    ...
    res.status(500).json({ message: 'Erro ao salvar carteira.' });
  }
};
...
const updateCarteira = async (req, res) => {
  const { name, balance, avgCpm, expiration, type } = req.body;
  const sql =
    'UPDATE milhas_carteiras SET nome=$1, saldo=$2, cpm_medio=$3, data_expiracao=$4, tipo=$5, atualizado_em=NOW() WHERE id=$6 AND user_id=$7 RETURNING *';
  try {
...
const deleteCarteira = async (req, res) => {
  try {
    await pool.query('DELETE FROM milhas_carteiras WHERE id=$1 AND user_id=$2', [
      req.params.id,
      req.usuario.id,
    ]);
    res.status(204).send();
...
const getMetas = async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM milhas_metas WHERE user_id = $1 ORDER BY criado_em DESC',
      [req.usuario.id]
    );
...
const createMeta = async (req, res) => {
  const { nomeDestino, origem, destino, programSuggestions, flightCostBRL, estimatedMiles } =
    req.body;
  const sql =
    'INSERT INTO milhas_metas (user_id, nome_destino, origem_sigla, destino_sigla, programa_alvo, custo_reais, custo_milhas) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *';
  try {
    const result = await pool.query(sql, [
      req.usuario.id,
      nomeDestino,
...
const updateMeta = async (req, res) => {
  const { nomeDestino, origem, destino, programSuggestions, flightCostBRL, estimatedMiles } =
    req.body;
  const sql =
    'UPDATE milhas_metas SET nome_destino=$1, origem_sigla=$2, destino_sigla=$3, programa_alvo=$4, custo_reais=$5, custo_milhas=$6, atualizado_em=NOW() WHERE id=$7 AND user_id=$8 RETURNING *';
  try {
    const result = await pool.query(sql, [
      nomeDestino,
      origem,
...
const deleteMeta = async (req, res) => {
  try {
    await pool.query('DELETE FROM milhas_metas WHERE id=$1 AND user_id=$2', [
      req.params.id,
      req.usuario.id,
    ]);
    res.status(204).send();
