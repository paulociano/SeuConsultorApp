...
const getAtivos = async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM ativos WHERE user_id = $1 ORDER BY tipo, nome', [req.usuario.id]);
...
const getDividas = async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM dividas WHERE user_id = $1 ORDER BY tipo, nome', [req.usuario.id]);
    res.json(result.rows);
