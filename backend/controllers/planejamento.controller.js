...
const getAposentadoria = async (req, res) => {
  try {
    const result = await pool.query('SELECT dados FROM aposentadoria_dados WHERE user_id = $1', [
      req.usuario.id,
    ]);
...
const getSimuladorPgbl = async (req, res) => {
  try {
    const result = await pool.query('SELECT dados FROM simulador_pgbl_dados WHERE user_id = $1', [
      req.usuario.id,
    ]);
    if (result.rows.length === 0) {
      return res.json(null);
...
const getAquisicoes = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT simulacoes FROM aquisicao_simulacoes WHERE user_id = $1 AND tipo_bem = $2',
      [req.usuario.id, tipo]
    );
    if (result.rows.length === 0) {
      return res.json([]);
