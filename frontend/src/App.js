...
const AppContent = () => {
    ...
    // Application data states (transacoes foi removido)
        ...
        const fetchInitialData = async () => {
            if (isAuthenticated) {
                ...
                try {
                    ...
                    // Endpoint 'transacoes' foi removido
                    const endpoints = [
                        'perfil', 'orcamento', 'aposentadoria',
                    ...
                    setUsuario({ ...perfilData.usuario, categorias: data[1] || [] });
                    setAposentadoriaData(data[2]);
                    setSimuladorPgblData(data[3]);
                    setAquisicaoData({ imoveis: data[4] || [], automoveis: data[5] || [] });
                    setMilhasData({ carteiras: data[6] || [], metas: data[7] || [] });
                    setAtas(data[8] || []);
                    setAgenda(data[9] || []);
    ...
    // --- CRUD Handlers (relacionados a transações foram removidos) ---
    ...
    const handleSaveOrcamentoItem = async (itemData, categoriaPaiId) => {
        ...
        const payload = {
            nome: itemData.nome, valor_planejado: itemData.sugerido, valor_atual: itemData.atual,
            categoria_planejamento: itemData.categoria_planejamento
        };
        if (!isEditing) { payload.categoria_id = categoriaPaiId; }
        ...
        if (itemSalvo) {
            setUsuario(currentUser => {
                ...
                if (!categoriaAlvo) { return currentUser; }
                if (isEditing) {
                ...
                return { ...currentUser, categorias: novasCategorias };
            });
    ...
    const { orcamentoCalculos, donutChartData } = useMemo(() => {
        ...
        const defaultReturn = { orcamentoCalculos: { totalReceitas: { atual: 0, sugerido: 0 }, totalDespesas: { atual: 0, sugerido: 0 }, saldoAtual: 0, saldoSugerido: 0 }, donutChartData: [] };
        if (!usuario || !usuario.categorias || usuario.categorias.length === 0) return defaultReturn;
        try {
            let totalReceitasAtual = 0, totalDespesasAtual = 0;
            const totaisDespesasPorTipo = { 'Fixos': 0, 'Variáveis': 0, 'Investimentos': 0, 'Proteção': 0, 'Outros': 0 };
            usuario.categorias.forEach(cat => {
                if (cat.tipo === 'receita') { totalReceitasAtual += cat.subItens.reduce((acc, item) => acc + (item.atual || 0), 0); return; }
            ...
            return { orcamentoCalculos: { totalReceitas: { atual: totalReceitasAtual, sugerido: 0 }, totalDespesas: { atual: totalDespesasAtual, sugerido: 0 }, saldoAtual, saldoSugerido: 0 }, donutChartData: newDonutChartData };
        } catch (e) { return defaultReturn; }
    }, [usuario.categorias]);

    const rendaMensal = useMemo(() => {
        if (!usuario.categorias) return 0;
        return usuario.categorias.filter(c => c.tipo === 'receita').reduce((acc, cat) => acc + cat.subItens.reduce((subAcc, item) => subAcc + (item.atual || 0), 0), 0);
    }, [usuario.categorias]);
            ...
            <main className={isAuthenticated ? "ml-64 p-4 md:p-6 transition-all duration-300" : ""}>
                <PageTransition key={location.pathname}>
                    <Routes>
                        ...
                        <Route path="/orcamento" element={<TelaOrcamento isLoading={isLoading} categorias={usuario.categorias || []} onSaveItem={handleSaveOrcamentoItem} onDeleteItem={handleDeleteOrcamentoItem} orcamentoCalculos={orcamentoCalculos} chartData={donutChartData} />} />
                        <Route path="/reserva" element={<TelaReservaEmergencia isLoading={isLoading} orcamento={usuario.categorias || []} investimentosSelecionados={reservaInvestimentos} onSelectionChange={setReservaInvestimentos} />} />
                        <Route path="/aposentadoria/aportes" element={<TelaAposentadoria isLoading={isLoading} dadosIniciais={aposentadoriaData} onSave={handleSaveAposentadoria} />} />
                        <Route path="/patrimonio" element={<TelaPatrimonio />} />
                        <Route 
                            path="/protecao" 
                            element={<TelaProtecao isLoading={isLoading} rendaMensal={rendaMensal} custoDeVidaMensal={custoDeVidaMensal} protecaoData={protecaoData} onSaveItem={handleSaveProtecaoItem} onDeleteItem={handleDeleteProtecaoItem} usuario={usuario} />} 
                        />
                        {/* A rota de fluxo de caixa agora é mais simples */}
                        <Route path="/fluxo/transacoes" element={<TelaFluxoDeCaixa isLoading={isLoading} />} />
                        <Route path="/fluxo/planejamento" element={<TelaPlanejamento isLoading={isLoading} orcamento={usuario.categorias} />} />
                        <Route path="/aquisicao/imoveis" element={<TelaAquisicaoImoveis isLoading={isLoading} dadosIniciais={aquisicaoData.imoveis} onSave={(data) => handleSaveAquisicao('imoveis', data)} />} />
                        <Route path="/aquisicao/automoveis" element={<TelaAquisicaoAutomoveis isLoading={isLoading} dadosIniciais={aquisicaoData.automoveis} onSave={(data) => handleSaveAquisicao('automoveis', data)} />} />
                        <Route path="/viagens/milhas" element={<TelaMilhas isLoading={isLoading} carteiras={milhasData.carteiras} metas={milhasData.metas} onSave={handleSaveMilhasItem} onDelete={handleDeleteMilhasItem} />} />
                        <Route path="/viagens/cartoes" element={<TelaCartoes isLoading={isLoading} />} />
                        <Route path="/educacao" element={<TelaEducacaoFinanceira isLoading={isLoading} />} />
                        <Route path="/aposentadoria/pgbl" element={<TelaSimuladorPGBL isLoading={isLoading} dadosIniciais={simuladorPgblData} onSave={handleSaveSimuladorPgbl} />} />
                        <Route path="/configuracoes" element={<TelaConfiguracoesPerfil usuario={usuario} setUsuario={setUsuario} />} />
                        <Route path="/agenda" element={<TelaReunioesAgenda isLoading={isLoading} atasIniciais={atas} agendaInicial={agenda} onSaveAta={handleSaveAta} onDeleteAta={handleDeleteAta} onSaveCompromisso={handleSaveCompromisso} onDeleteCompromisso={handleDeleteCompromisso} />} />
                        
                        <Route path="*" element={isAuthenticated ? <TelaObjetivos /> : <TelaAutenticacao setIsAuthenticated={setIsAuthenticated} setUsuario={setUsuario} />} />
                    </Routes>
                </PageTransition>
            </main>
            {/* O modal de transação foi removido daqui */}
        </div>
    );
};
