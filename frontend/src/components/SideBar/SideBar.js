...
import {
    ...
    ChartLine, Plane, CalendarDays, Sun, Moon, ChevronLeft, Settings
} from 'lucide-react';
...
export default function Sidebar({ isCollapsed, setIsCollapsed }) {
    ...
    return (
        ...
        >
            ...
            <div className={clsx("flex items-center p-4 border-b border-slate-200 dark:border-[#3e388b] transition-all", isCollapsed ? "justify-center" : "justify-between")}>
                <div className="flex items-center gap-2 overflow-hidden">
                    <img src={logo} alt="Logo SeuConsultor" className={clsx("flex-shrink-0 transition-all duration-300", isCollapsed ? "h-8 w-8" : "h-6 w-6")} style={{ filter: 'invert(42%) sepia(93%) saturate(2000%) hue-rotate(133deg) brightness(100%) contrast(107%)' }} />
                    <span className={clsx("font-montserrat text-slate-900 dark:text-white text-sm font-extrabold transition-opacity duration-200", isCollapsed && "opacity-0")}>
                        SeuConsultor
                    </span>
                </div>
                <button onClick={toggleTheme} className={clsx("p-2 rounded-full text-gray-500 dark:text-gray-300 hover:bg-slate-200 dark:hover:bg-[#3e388b]/50 transition-opacity", isCollapsed && "opacity-0 pointer-events-none")}>
                            ...
                            <div key={item.id}>
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={clsx("whitespace-nowrap transition-opacity", isCollapsed && "opacity-0")}>{item.label}</span>
                                    </div>
                                    {item.subItems && <ChevronDown size={14} className={clsx("transform transition-all", openMenu === item.id ? 'rotate-180' : '', isCollapsed && "opacity-0")} />}
                                </button>
            ...
            <div className="border-t border-slate-200 dark:border-[#3e388b] p-3">
                <button 
                    onClick={isCollapsed ? () => navigate('/configuracoes') : () => setPerfilAberto(prev => !prev)} 
                    className={clsx(
                    ...
                    {isCollapsed ? (
                        ...
                        <>
                            <div className="flex items-center gap-3 overflow-hidden">
                                <img src={usuario?.imagem_url || `https://ui-avatars.com/api/?name=${usuario?.nome || 'U'}&background=00d971&color=000`} alt="avatar" className="w-8 h-8 rounded-full object-cover border flex-shrink-0" />
                                <span className="text-sm font-medium text-slate-800 dark:text-white whitespace-nowrap">{usuario?.nome}</span>
                            </div>
                            <ChevronDown size={16} className={clsx("transition-transform duration-300 flex-shrink-0", !perfilAberto ? 'rotate-180' : 'rotate-0')} />
                        </>
                    )}
                </button>
                {!isCollapsed && (
                     <div className={`grid transition-all duration-300 ease-in-out ${perfilAberto ? 'grid-rows-[1fr] opacity-100 mt-2' : 'grid-rows-[0fr] opacity-0 mt-0'} overflow-hidden`}>
                        <div className="min-h-0">
                            <div className="space-y-2 pl-11">
                                <button onClick={() => { navigate('/configuracoes'); setPerfilAberto(false); }} className="w-full text-left text-sm text-gray-600 dark:text-gray-300 hover:underline">Configurações</button>
