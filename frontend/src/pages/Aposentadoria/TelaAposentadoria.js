...
const TelaAposentadoria = () => {
  const { aposentadoriaData, isLoading, fetchAposentadoria, saveAposentadoria } =
    useAposentadoriaStore();

  // --- INÍCIO DA LÓGICA DO ONBOARDING (CORRIGIDA) ---
  const TOUR_KEY = 'aposentadoria_tour';
  ...
  const tourSteps = [
    {
      target: 'body',
      content:
        'Bem-vindo ao Simulador de Aposentadoria! Preencha os campos para projetar seu futuro financeiro.',
      placement: 'center',
      disableBeacon: true,
    },
    {
      target: '#parametros-iniciais-card',
      content:
        'Comece por aqui. Insira seus dados atuais, suas metas e a rentabilidade esperada dos seus investimentos.',
    },
    {
      target: '#plano-aportes-card',
      content:
        'Você pode definir aportes mensais específicos para os próximos anos ou um valor padrão para todo o período.',
    },
    {
      target: '#projecao-card',
      content:
        'O resultado da sua simulação aparecerá aqui! Este gráfico mostra a evolução do seu patrimônio ao longo do tempo.',
    },
    {
      target: '#salvar-simulacao-btn',
      content:
        'Quando estiver satisfeito com a simulação, não se esqueça de salvar para consultar mais tarde.',
    },
  ];
  ...
  useEffect(() => {
    if (aposentadoriaData) {
      ...
      setAportes(
        aposentadoriaData.aportes?.map((a) => ({ ...a, id: a.id || uuidv4() })) || [
          { id: uuidv4(), ano: 1, valor: 1000 },
        ]
      );
  ...
  const { projectionData, capitalNecessario, valorAcumulado } = useMemo(() => {
    ...
    return {
      projectionData: data,
      capitalNecessario: capitalNecessarioCalc,
      valorAcumulado: valorFinalAcumulado,
    };
  }, [
    idadeAtual,
    idadeAposentadoria,
  ...
  const handleAporteChange = (id, novoValor) => {
    setAportes((prev) =>
      prev.map((a) => (a.id === id ? { ...a, valor: parseFloat(novoValor) || 0 } : a))
    );
  };
  ...
  const addAporteAno = () => {
    setAportes((prev) => {
      ...
      return [
        ...prev,
        { id: uuidv4(), ano: proximoAno, valor: prev[prev.length - 1]?.valor || 1000 },
      ];
    });
  };
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          <Card id="parametros-iniciais-card">
            <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-4">
              Parâmetros Iniciais
            </h2>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <label className="block font-medium text-slate-800 dark:text-white">
                  Idade Atual
                </label>
                <input
                  type="number"
                  ...
                  className="mt-1 w-full bg-white dark:bg-[#2a246f] text-slate-900 dark:text-white rounded-md px-3 py-2 border border-slate-300 dark:border-[#3e388b]"
                />
              </div>
              <div>
                <label className="block font-medium text-slate-800 dark:text-white">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPatrimonioInicial(parseFloat(e.target.value) || 0)}
                  className="mt-1 w-full bg-white dark:bg-[#2a246f] text-slate-900 dark:text-white rounded-md px-3 py-2 border border-slate-300 dark:border-[#3e388b]"
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  value={rendaDesejada}
                  onChange={(e) => setRendaDesejada(parseFloat(e.target.value) || 0)}
                  className="mt-1 w-full bg-white dark:bg-[#2a246f] text-slate-900 dark:text-white rounded-md px-3 py-2 border border-slate-300 dark:border-[#3e388b]"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setRentabilidadeAnual(parseFloat(e.target.value) || 0)}
                  className="mt-1 w-full bg-white dark:bg-[#2a246f] text-slate-900 dark:text-white rounded-md px-3 py-2 border border-slate-300 dark:border-[#3e388b]"
                />
              </div>
              <div>
                ...
                <select
                  value={tipoPrevidencia}
                  onChange={(e) => setTipoPrevidencia(e.target.value)}
                  className="mt-1 w-full bg-white dark:bg-[#2a246f] text-slate-900 dark:text-white rounded-md px-3 py-2 border border-slate-300 dark:border-[#3e388b]"
                >
          ...
          <Card id="plano-aportes-card">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">Plano de Aportes</h2>
              <button
                onClick={addAporteAno}
                disabled={anosRestantes <= 0}
                className="text-xs flex items-center gap-1 text-[#00d971] hover:brightness-90 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <PlusCircle size={14} /> Adicionar Ano
              </button>
            </div>
              ...
              {aportes.map((aporte) => (
                <div key={aporte.id} className="flex items-center gap-4 text-sm">
                  ...
                  <input
                    type="number"
                    value={aporte.valor}
                    onChange={(e) => handleAporteChange(aporte.id, e.target.value)}
                    className="flex-1 bg-white dark:bg-[#2a246f] text-slate-900 dark:text-white rounded-md px-3 py-2 border border-slate-300 dark:border-[#3e388b]"
                  />
                </div>
              ))}
              {anosRestantes > 0 && (
                <div className="flex items-center gap-4 text-sm pt-2 border-t border-slate-200 dark:border-[#3e388b]">
                  <label className="text-slate-800 dark:text-white w-28">
                    Restante ({anosRestantes} anos):
                  </label>
                  <input
                    type="number"
                    value={aporteRestante}
                    onChange={(e) => setAporteRestante(parseFloat(e.target.value) || 0)}
                    className="flex-1 bg-white dark:bg-[#2a246f] text-slate-900 dark:text-white rounded-md px-3 py-2 border border-slate-300 dark:border-[#3e388b]"
                  />
                </div>
              )}
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={projectionData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                ...
                <Tooltip
                  contentStyle={{
                    backgroundColor: '#2a246f',
                    border: '1px solid #3e388b',
                    borderRadius: '0.5rem',
                  }}
          ...
          <div className="grid grid-cols-2 gap-4 mt-4 text-center">
            <div className="bg-slate-100 dark:bg-[#201b5d]/50 p-3 rounded-lg">
              ...
              <p className="text-xl font-bold text-slate-800 dark:text-white mt-1">
                {formatCurrency(capitalNecessario)}
              </p>
            </div>
            <div className="bg-slate-100 dark:bg-[#201b5d]/50 p-3 rounded-lg">
              <p className="text-sm text-slate-600 dark:text-white">Você terá</p>
              <p className="text-xl font-bold text-[#00d971] mt-1">
