...
const TabButton = ({ label, active, onClick, ...props }) => (
  <button
    {...props}
    onClick={onClick}
    className={`px-4 py-2 rounded-t-lg font-medium transition-colors duration-300 ${active ? 'bg-[#201b5d] text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600'}`}
  >
    {label}
  </button>
);
...
const TelaSimuladorPGBL = () => {
  const { simuladorPgblData, isLoading, fetchAposentadoria, saveSimuladorPgbl } =
    useAposentadoriaStore();

  // --- INÍCIO DA LÓGICA DO ONBOARDING ---
  const TOUR_KEY = 'simulador_pgbl_tour';
  ...
  const tourSteps = [
    {
      target: 'body',
      content:
        'Bem-vindo ao Simulador PGBL/VGBL! Esta ferramenta ajuda a decidir qual plano de previdência é melhor para você.',
      placement: 'center',
      disableBeacon: true,
    },
    {
      target: '#pgbl-entradas-card',
      content:
        'Comece preenchendo seus dados financeiros anuais aqui, como renda bruta e despesas dedutíveis.',
    },
    {
      target: '#pgbl-resultados-tab',
      content:
        'Depois de preencher seus dados, clique na aba "Resultados" para ver a mágica acontecer!',
    },
    {
      target: '#pgbl-comparativo-card',
      content:
        'Aqui você verá a comparação entre os modelos e qual produto é sugerido para você, além da economia de imposto!',
      // Este passo depende da aba 'Resultados' estar ativa.
      // O Joyride tentará encontrá-lo, o usuário pode precisar clicar na aba para continuar.
    },
    {
      target: '#pgbl-save-btn',
      content: 'Quando terminar, não se esqueça de salvar seus dados para consultas futuras.',
    },
  ];
  ...
  const resultado = useMemo(() => {
    const totalDeducoes =
      inss + educacao + saude + outras + numDependentes * DEDUCAO_DEPENDENTE_MENSAL * 12;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <div id="pgbl-tabs-container" className="flex space-x-2">
          <TabButton
            label="Entradas"
      ...
      {activeTab === 'Entradas' && (
        <AnimatedCard id="pgbl-entradas-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 text-center">
          <AnimatedCard>
            ...
            <p className="text-slate-600 dark:text-slate-300">
              Base de Cálculo: {formatCurrency(resultado.baseCompleta)}
            </p>
            <p className="text-slate-600 dark:text-slate-300">
              IR Devido: {formatCurrency(resultado.irCompleta)}
            </p>
            <p
              className={`mt-2 font-bold ${resultado.resultadoFinalCompleta > 0 ? 'text-red-500' : 'text-green-500'}`}
            >
              {resultado.resultadoFinalCompleta > 0
                ? `A Pagar: ${formatCurrency(resultado.resultadoFinalCompleta)}`
                : `A Restituir: ${formatCurrency(-resultado.resultadoFinalCompleta)}`}
            </p>
          </AnimatedCard>
          <AnimatedCard>
            <h3 className="font-bold text-slate-800 dark:text-white mb-2">Modelo Simplificado</h3>
            <p className="text-slate-600 dark:text-slate-300">
              Base de Cálculo: {formatCurrency(resultado.baseSimplificada)}
            </p>
            <p className="text-slate-600 dark:text-slate-300">
              IR Devido: {formatCurrency(resultado.irSimplificada)}
            </p>
            <p
              className={`mt-2 font-bold ${resultado.resultadoFinalSimplificada > 0 ? 'text-red-500' : 'text-green-500'}`}
            >
              {resultado.resultadoFinalSimplificada > 0
                ? `A Pagar: ${formatCurrency(resultado.resultadoFinalSimplificada)}`
                : `A Restituir: ${formatCurrency(-resultado.resultadoFinalSimplificada)}`}
            </p>
          </AnimatedCard>
          <AnimatedCard id="pgbl-comparativo-card">
            <h3 className="font-bold text-slate-800 dark:text-white mb-2">
              Comparativo e Sugestão
            </h3>
            <p className="text-slate-600 dark:text-slate-300">
              Melhor Produto: <strong className="text-[#00d971]">{resultado.melhorProduto}</strong>
            </p>
            <p className="text-slate-600 dark:text-slate-300">
              Economia de Imposto:{' '}
              <strong className="text-blue-400">{formatCurrency(resultado.economiaFinal)}</strong>
            </p>
            <p className="text-slate-600 dark:text-slate-300">
              Limite PGBL (12%):{' '}
              <strong className="text-yellow-400">
                {formatCurrency(resultado.pgblMaxPermitido)}
              </strong>
            </p>
          </AnimatedCard>
        </div>
