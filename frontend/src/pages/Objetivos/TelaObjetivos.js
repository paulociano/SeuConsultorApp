    ...
    >
      ...
      >
        <div className="flex flex-col items-center text-center p-3">
          <div
            className={`w-14 h-14 mb-3 rounded-full flex items-center justify-center border-2 ${unlocked ? 'border-yellow-400 bg-yellow-400/20' : 'border-gray-300 dark:border-gray-600'}`}
          >
            <Icon
              size={28}
              className={unlocked ? 'text-yellow-500' : 'text-gray-400 dark:text-gray-500'}
            />
          </div>
          <h4
            className={`font-semibold text-sm ${unlocked ? 'text-slate-800 dark:text-white' : 'text-gray-500'}`}
          >
            {achievement.name}
          </h4>
          <p className="text-xs text-gray-500 dark:text-gray-400 h-8 mt-1">
...
const TelaObjetivos = () => {
  ...
  const tourSteps = [
    ...
    },
    {
      ...
      content:
        'Você pode alternar entre a visualização "Gamificada", com cards, e a "Clássica", com uma visão orbital de suas metas.',
    },
    {
      target: '#main-view-container',
      content:
        'Nesta área principal, você verá seus objetivos. Interaja com eles para ver mais detalhes.',
    },
    {
      target: '#conquistas-gallery',
      content:
        'Ao completar objetivos, você desbloqueia conquistas que aparecerão aqui. Colecione todas!',
    },
    {
  ...
  const metaSelecionada = useMemo(
    () => objetivosCalculados.find((o) => o.id === objetivoSelecionadoId),
  ...
  const progressoMetaDetalhe = useMemo(() => {
    ...
    return {
      ...
      data: [
        ...
        {
          name: 'Faltante',
          value: Math.max(0, metaSelecionada.valorAlvo - metaSelecionada.valorAtual),
        },
      ],
    };
  }, [metaSelecionada]);
    ...
    return (
      <div className="flex flex-col items-center justify-center pt-4 relative min-h-[500px] mb-4">
        <div
          ...
          style={{
            width: `${(raioBase + objetivosPorAnel.length * 80) * 2}px`,
            height: `${(raioBase + objetivosPorAnel.length * 80) * 2}px`,
          }}
        >
          <div className="absolute z-30">
            <img
              src={userImage}
              alt="Usuário"
              className="w-24 h-24 rounded-full border-4 border-white dark:border-[#201b5d] shadow-lg"
            />
          </div>
          {objetivosPorAnel.map((anel, anelIndex) => {
            ...
            return (
              <React.Fragment key={anelIndex}>
                <div
                  className="absolute top-1/2 left-1/2 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-full animate-spin-slow pointer-events-none"
                  style={{
                    width: `${raioAtual * 2}px`,
                    height: `${raioAtual * 2}px`,
                ...
                {anel.map((objetivo, objIndex) => {
                  ...
                  const porcentagemAtingida =
                    objetivo.valorAlvo > 0 ? (objetivo.valorAtual / objetivo.valorAlvo) * 100 : 0;
                  return (
                    <div
                      key={objetivo.id}
                    ...
                    >
                          ...
                          <PieChart>
                            <Pie
                              data={[
                                ...
                                { value: Math.max(0, objetivo.valorAlvo - objetivo.valorAtual) },
                              ]}
                              dataKey="value"
                              startAngle={90}
                              endAngle={-270}
                            ...
                            >
                              <Cell fill="#00d971" />
                              <Cell fill={theme === 'dark' ? '#2a246f' : '#e2e8f0'} />
                            </Pie>
                          </PieChart>
                        </ResponsiveContainer>
                        <motion.button
                      ...
                      <p className="mt-2 text-xs font-semibold text-center text-slate-600 dark:text-gray-400 w-24 truncate">
                        {objetivo.nome}
                      </p>
                    </div>
                  );
          ...
          return (
            ...
            >
                  ...
                  <AnimatePresence mode="wait">
                    {hoveredObjectiveId === obj.id ? (
                      <motion.span
                        ...
                        exit={{ opacity: 0, scale: 0.8 }}
                        transition={{ duration: 0.2 }}
                        className="text-sm font-bold text-[#00d971] w-6 h-6 flex items-center justify-center"
                      >
              ...
              <p className="text-sm text-gray-500 dark:text-gray-300 mt-1">
                {formatCurrency(obj.valorAtual)} de {formatCurrency(obj.valorAlvo)}
              </p>
              <div className="relative w-full h-3 bg-slate-200 dark:bg-slate-700 rounded mt-2">
                <div
                  className="absolute top-0 left-0 h-3 rounded bg-[#00d971]"
                  style={{ width: `${Math.min(porcentagem, 100)}%` }}
                ></div>
              </div>
              {obj.aporteMensal > 0 && (
                <p className="text-xs mt-1 text-gray-400">
                  Aporte: {formatCurrency(obj.aporteMensal)} / mês
                </p>
              )}
            </motion.div>
          );
    ...
    return (
      ...
      >
        <div className="max-w-xl mx-auto">
          <button
            onClick={() => setObjetivoSelecionadoId(null)}
            className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white mb-4"
          >
          ...
          <Card>
            <div className="flex flex-col items-center text-center">
              <div className="flex justify-end w-full gap-4 -mt-2 -mr-2">
                <button
                  onClick={() => handleOpenModal(metaSelecionada)}
                  className="text-gray-500 dark:text-gray-400 hover:text-blue-500 transition-colors"
                >
                  <Pencil size={18} />
                </button>
                <button
                  onClick={() => handleDeleteObjetivo(metaSelecionada.id)}
              ...
              <p className="text-lg text-gray-500 dark:text-gray-400 mt-1">
                Alcançado:{' '}
                <span className="font-bold text-[#00d971]">
                  {formatCurrency(metaSelecionada.valorAtual)}
                </span>{' '}
                de {formatCurrency(metaSelecionada.valorAlvo)}
              </p>
              <div className="relative w-48 h-48 my-4">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={progressoMetaDetalhe.data}
                      dataKey="value"
                      nameKey="name"
                      cx="50%"
                      cy="50%"
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-3xl font-bold text-slate-800 dark:text-white">
                    {progressoMetaDetalhe.percentual.toFixed(0)}%
                  </span>
                </div>
              </div>
              <div className="w-full text-left mt-4">
                ...
                <div className="space-y-2">
                  {metaSelecionada.investimentosLinkados.length > 0 ? (
                    metaSelecionada.investimentosLinkados.map((invId) => {
                      const investimento = ativos.find((i) => i.id === invId);
                      return (
                        <div
                          key={invId}
                          className="flex justify-between items-center p-3 bg-slate-100 dark:bg-[#2a246f] rounded-lg"
                        >
                          <span className="font-medium text-slate-700 dark:text-gray-300">
                            {investimento?.nome || 'Investimento não encontrado'}
                          </span>
                          <span className="font-bold text-slate-800 dark:text-white">
                            {investimento ? formatCurrency(investimento.valor) : ''}
                          </span>
                        </div>
                      );
                    })
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 text-sm bg-[#00d971] text-black font-bold px-4 py-1.5 rounded-lg hover:brightness-90 transition-colors"
          >
            <PlusCircle size={16} />
        ...
        {objetivoSelecionadoId ? (
          ...
          <div key="empty-state" className="text-center py-20">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              Clique no botão <span className="text-[#00d971] font-bold">+</span> para começar a
              planejar seu futuro!
            </p>
          </div>
        )}
