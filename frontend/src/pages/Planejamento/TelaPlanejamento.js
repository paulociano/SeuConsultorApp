...
const TelaPlanejamento = () => {
    ...
    useEffect(() => {
        fetchOrcamento();
        fetchTransacoes();
    }, [fetchOrcamento, fetchTransacoes]);
    ...
    const dadosPlanejamento = useMemo(() => {
        // 3. CORREÇÃO: Usar a variável 'categorias' em vez de 'orcamento'
        const metasAgrupadas = (categorias || [])
            .filter(cat => cat.tipo === 'despesa')
            .flatMap(cat => cat.subItens)
            .reduce((acc, item) => {
                const categoriaPlanejamento = item.categoria_planejamento;
                if (categoriaPlanejamento) {
                    acc[categoriaPlanejamento] = (acc[categoriaPlanejamento] || 0) + Number(item.sugerido || 0);
                }
                return acc;
            }, {});
        ...
        const gastosAgrupados = (gastosReais || [])
            .filter(t => {
                ...
                return (t.tipo === 'despesa' || t.tipo === 'debit') && !t.ignorada && data.getFullYear() === filtroData.ano && (data.getMonth() + 1) === filtroData.mes;
            })
            .reduce((acc, t) => {
                const categoriaGasto = t.categoria;
                if (categoriaGasto) {
                    acc[categoriaGasto] = (acc[categoriaGasto] || 0) + Number(t.valor || 0);
                }
                return acc;
            }, {});
    ...
    // 4. CORREÇÃO: Adicionar 'categorias' ao array de dependências do useMemo
    }, [categorias, gastosReais, filtroData]);
    ...
    const isLoading = isOrcamentoLoading || isTransacoesLoading;
